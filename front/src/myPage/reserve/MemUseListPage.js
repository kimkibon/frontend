                                    ...
                                    <div class="align-items-center align-content-center col-md-2 border-left mt-1">
                                        <div class="d-flex flex-column mt-4">
                                            ...
                                            {review_check === 1 ?
                                                ...
                                                :  <div>
                                                    <button type="button" class="btn btn-primary m-1" onClick={()=>{setReviewShow(true)}}>리뷰확인</button>                                        
                                            ...
                                            {new Date().getTime()<after_date ?
                                                                        (review_check===0 ? 
                                                                            <button type="button" class="btn btn-info m-1" style={{width:90+'px'}}
                                                                                    onClick={()=>{setResidx(list.RES_IDX);setBoardno(list.RES_BOARD_NO);setModalShow(true);}}>리뷰쓰기</button>
                                                                                            :<Link to="" className="disable-link"><button type="button" class="btn btn-outline-info m-1" disabled>리뷰쓰기</button></Link>)
...
const NoReviewModal=(props)=>{
    return (
        ...
        >
            ...
            <Modal.Footer>
                <button type="button" class="btn btn-secondary "  onClick={props.onHide}>확인</button>
            </Modal.Footer>
        </Modal>
      )
}
